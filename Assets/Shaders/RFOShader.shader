    ...
    SubShader
    {
        ...
        Pass // 5
        {
            ...
            half4 frag (v2f i) : SV_Target {
                ...
                float sceneDepth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.pos.xy / _ScreenParams.xy);
                float linearSceneDepth = LinearEyeDepth(sceneDepth);
    
                // Convert object position to depth space
                float linearMaskDepth = LinearEyeDepth(i.pos.z);

                // Only render the outline if it's visible
                if (linearMaskDepth > linearSceneDepth - 0.01)
                {
                    discard; // Ignore outline if behind another object
                }
